    ...
    property_decider(options, ui_message_handler, equation, ns)
{
  ...
  property_decider.get_solver()
    .decision_procedure_incremental()
    .set_all_frozen();
}
  ...
  while(has_properties_to_check(properties))
  {
    ...
    if(count_properties(properties, property_statust::UNKNOWN) > 0)
    {
      ...
      if(!current_equation_converted)
      {
        ...
        property_decider.get_solver()
          .decision_procedure_contexts()
          .push_context();
        equation.convert_assertions(
          property_decider.get_solver().decision_procedure(), false);
      ...
      // Let's pop them.
      property_decider.get_solver().decision_procedure_contexts().pop_context();
    }
